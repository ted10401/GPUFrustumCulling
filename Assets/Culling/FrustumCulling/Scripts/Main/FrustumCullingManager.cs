...
namespace FS2.FrustumCulling
{
    public class FrustumCullingManager : MonoSingleton<FrustumCullingManager>
    {
        ...
        private const float EXTENTS_MULTIPLIER = 1f;
        ...
        private void UpdateCPUFrustumCullings()
        {
            if(m_cpuFrustumCullings.Count > 0)
            {
                foreach (FrustumCullingBase frustumCullingBase in m_cpuFrustumCullings.Values)
                {
                    ...
                    m_cacheReseult = FrustumCullingTest(m_frustumVector4, frustumCullingBase.center, frustumCullingBase.extents);
        ...
        private bool FrustumCullingTest(Vector4[] planes, Vector3 center, Vector3 extents)
        {
            return FrustumCullingUtility.TestPlanesAABBInternalFast(planes, center, extents * EXTENTS_MULTIPLIER);
        }
